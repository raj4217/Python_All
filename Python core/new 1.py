...
class mngr(empl):
    def __init__(self, fname, lname, pay, empls =None):
        super().__init__(fname, lname, pay)
        self.empls = [] if empls == None else self.empls = empls

    def addEmp(self, empls):
        if empls not in self.empls: self.empls.append(empls)

    def rmvEmp(self, empls):
        if empls in self.empls: self.empls.remove(empls)

    def printEmpls(self):
        for e in self.empls: print('\t'+ e.fullName())
